...
//    List<Coupon1> findByCustomerIdAndCategory(int customerId, Category category);
    ...
    @Query(value = "SELECT * FROM coupons c WHERE c.id in (SELECT coupons_id FROM customers_coupons where customer_id = ?1)", nativeQuery = true)
    List<Coupon1> findCustomerCoupons(int customerId);

    @Query(value = "SELECT * FROM coupons where category = ?2 and id in (SELECT coupons_id FROM customers_coupons WHERE customer_id = ?1) ", nativeQuery = true)
    List<Coupon1>  findCustomerCouponsByCategory(int customerId, int categoryId);

    @Query(value = "SELECT * FROM coupons where price < ?2 and id in (SELECT coupons_id FROM customers_coupons WHERE customer_id = ?1) ", nativeQuery = true)
